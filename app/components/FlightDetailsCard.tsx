import { Plane, Clock, MapPin, CheckCircle2, AlertTriangle, XCircle, Info } from "lucide-react";
...
export default function FlightDetailsCard({ flight }: FlightDetailsCardProps) {
  ...
  return (
    <div className="glass-card rounded-3xl p-6 md:p-8 animate-fade-in w-full">
      <div className="flex flex-wrap items-center justify-between mb-8 pb-6 border-b border-slate-200 dark:border-slate-800">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-3">
            <Plane className={`w-6 h-6 ${isCancelled ? 'text-red-500' : 'text-blue-500'}`} />
            {flight.airline} {flight.flight_number}
          </h2>
        </div>
        <div className={`mt-4 sm:mt-0 flex items-center gap-2 px-4 py-2 rounded-full border text-sm font-semibold capitalize ${getStatusColor(flight.status)}`}>
      ...
      <div className="relative flex justify-between items-start md:items-center flex-col md:flex-row gap-8 md:gap-0">
        ...
        <div className="flex-1 w-full md:w-auto">
          ...
          <p className="text-4xl font-bold text-slate-900 dark:text-white mb-2">{flight.departure.iata}</p>
        ...
        <div className="w-full md:flex-1 md:px-8 py-6 md:py-0">
          <div className="w-full flex items-center relative">
            <div className={`w-3 h-3 rounded-full shrink-0 z-10 ${isCancelled ? 'bg-red-500 ring-red-500/20' : 'bg-blue-500 ring-blue-500/20'} ring-4`} />
            ...
            <Plane 
              className={`w-6 h-6 absolute top-1/2 -translate-y-1/2 -mt-3 transition-all duration-1000 ease-in-out ${isCancelled ? 'text-red-500' : 'text-blue-500'}`} 
              style={{ left: `calc(${getProgressWidth(flight.status)} - 12px)` }}
            />

            <div className={`w-3 h-3 rounded-full shrink-0 z-10 ${flight.status === 'landed' ? 'bg-green-500 ring-4 ring-green-500/20' : 'bg-slate-300 dark:bg-slate-600'}`} />
          </div>
          <div className="text-center mt-6">
            <p className="text-xs text-slate-400 font-medium uppercase tracking-wider inline-flex items-center gap-1 bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full">
        ...
        <div className="flex-1 w-full md:w-auto text-left md:text-right">
          ...
          <p className="text-4xl font-bold text-slate-900 dark:text-white mb-2">{flight.arrival.iata}</p>
          ...
          <div className="mt-4 space-y-2 flex flex-col md:items-end">
            <div className="flex items-center md:flex-row-reverse gap-2 text-slate-600 dark:text-slate-400">
              <Clock className="w-4 h-4 shrink-0" />
